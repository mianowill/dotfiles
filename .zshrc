...
# powerline
if [ -z "$POWERLINE_LOC" ]; then
    POWERLINE_LOC=/opt/homebrew/lib/python3.10/site-packages/powerline
fi
...
if [ -n "$SOURCE_IDF" ]; then
    ...
    # select a random cow from /opt/homebrew/share/cows (requires shuf from coreutils)
    export COW=$(ls /opt/homebrew/share/cows | shuf -n 1)
    cowsay -f $COW $(fortune)
fi

# add espressif llvm to path
export PATH="/opt/esp-clang/bin:$PATH"
